...
ccps <- left_join(x = result_new, y = fips_codes, by = c("fips" = "county_fips")) # join crop returns data to state IDs
    ...
    for(k in initial_uses){
      for(l in final_uses){
        ...
        ggplot(data = world) + # map US counties
          ...
          scale_fill_manual("CCP", values=c("(0, 0.111)" = "#F7FCF5", "(0.111, 0.222)" = "#E5F5E0", "(0.222, 0.333)" = "#C7E9C0", "(0.333, 0.444)" = "#A1D99B", "(0.444, 0.556)" = "#74C476", "(0.556, 0.667)" = "#41AB5D", "(0.667, 0.778)" = "#238B45", "(0.778, 0.889)" = "#006D2C", "(0.889, 0.999)" = "#00441B", "1" = "#ffa500", "NaN" = "#B8B8B8")) + # set manual color scale
        ...
        ggsave(sprintf("results/initial_descriptives/combined/ccps_troubleshooting/ccp_%s_%s_%s_%s.png", toString(i), j, k, l), width = 15, height = 8.33, dpi=96) # save map
