...
********************************************************************************
* import
import delimited raw_data\NASS\qs.economics_20201117.txt, clear
...
* save
compress
save processing\NASS\pasturerents_all, replace
use processing\NASS\pasturerents_all, clear

********************************************************************************
************MAKE A BLANK COUNTY PANEL************
**************with asd (agricultural stastitics district)***********************
* county-asd dictionary
	import excel "raw_data\NASS\Listing of Counties and Districts used by USDA-NASS.xlsx", sheet("data") firstrow clear
	* generate fips code
	...
	* save
	compress
	save processing\NASS\county_list, replace
* merge county-asd dictionary to data counties
	...
	duplicates drop
	merge 1:1 state_fips_code asd_code fips using processing\NASS\county_list
...
* merge in multi-state regions
	import excel "raw_data\NASS\Farm Production Regions.xlsx", sheet("Sheet1") firstrow clear
	merge 1:m state_alpha using processing\NASS\county_list
	assert _merge == 3
	drop _merge
	* save
	compress
...
* year list
	...
	local nyears = r(N)
	di `nyears'
	save processing\NASS\year_list, replace
* merge to years
	use processing\NASS\county_list, clear
	expand `nyears'
	bysort fips: gen n1 = _n
	merge m:1 n1 using processing\NASS\year_list
	assert _merge == 3
	drop _merge n1
	compress
	order multistateregion_desc state_fips_code state* asd* fips county* year
	save processing\NASS\county_panel_empty, replace
	use processing\NASS\county_panel_empty, clear
* clean
	erase processing\NASS\county_list.dta
	erase processing\NASS\year_list.dta
...
* 1 - county-level data: merge county-level data to blank panel
	...
	merge 1:1 fips year using processing\NASS\county_panel_empty
...
/* 2 - other counties in the asd: 
	...
	* merge to county dataset by state, year, and asd
	merge 1:m state_fips_code asd_code year using  processing\NASS\pasturerents
...
* 4 - merge in state-level data
	...
	* merge to working dataset
	merge 1:m state_fips_code year using processing\NASS\pasturerents
...
* 5 - multi-state region level data
	...
	* merge to working dataset
	merge 1:m multistateregion_desc year using processing\NASS\pasturerents
